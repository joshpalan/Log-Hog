...
class App
{
    ...
    public function run()
    {
        ...
        $filesThatWeNeed = array(
            'core/conf/config.php',
            'core/js/jquery.js',
            'core/php/configStatic.php',
            'core/php/loadVars.php',
            'core/php/loadVarsToJs.php',
            'core/php/updateCheck.php',
            'core/php/customCSS.php',
            'core/php/template/popup.php',
            'core/template/loading-bar.css',
            'core/js/loading-bar.min.js',
            'core/js/main.js',
            'core/js/rightClickJS.js',
            'core/js/update.js',
            'setup/setupProcessFile.php',
            'error.php'
        );

        ErrorCheck::checkIfFilesExist($filesThatWeNeed, '', $currentPage);
        ErrorCheck::checkIfFilesAreReadable($filesThatWeNeed, '', $currentPage);
        self::setCookieRedirect();
        $currentSelectedTheme = self::returnCurrentSelectedTheme();
        $baseLayoutUrl = 'local/' . $currentSelectedTheme . '/';

        if(!file_exists($baseLayoutUrl.'conf/config.php'))
        {
        ...
        require_once($baseLayoutUrl.'conf/config.php');
     ...
     * @return void
     */
    private static function setCookieRedirect($customUrl = null): void
    {
    ...
    function generateRestoreList($configStatic)
    {
        ...
        foreach ($versionList as $key => $value) {
            $versionListText = "<option value='" . str_replace('Update', '',
                    $value['branchName']) . "' >" . $key . '</option>' . $versionListText;
        }
    ...
    function upgradeConfig($newSaveStuff = array())
    {
        ...
        $baseUrl = LOGHOG_BASE_DIR . 'local/';
        include($baseUrl . 'layout.php');
        $baseUrl .= $currentSelectedTheme . '/';
        include($baseUrl . 'conf/config.php');
        include(LOGHOG_BASE_DIR . 'core/conf/config.php');
        $currentTheme = loadSpecificVar($defaultConfig, $config, 'currentTheme');
        if (is_dir(LOGHOG_BASE_DIR . 'local/' . $currentSelectedTheme . '/Themes/' . $currentTheme)) {
            include(LOGHOG_BASE_DIR . 'local/' . $currentSelectedTheme . '/Themes/' . $currentTheme . '/defaultSetting.php');
        } else {
            include(LOGHOG_BASE_DIR . 'core/Themes/' . $currentTheme . '/defaultSetting.php');
        }
        include(LOGHOG_BASE_DIR . 'core/php/loadVars.php');
        ...
        foreach ($defaultConfig as $key => $value) {
            if (isset($newSaveStuff[$key])) {
                ...
                (
                    ...
                    isset($themeDefaultSettings) && array_key_exists($key,
                        $themeDefaultSettings) && $themeDefaultSettings[$key] !== $$key
                )
                ||
                $$key === $defaultConfig[$key] && isset($themeDefaultSettings) && array_key_exists($key,
                    $themeDefaultSettings) && $themeDefaultSettings[$key] !== $$key
                ||
    ...
    protected static function returnCurrentSelectedTheme()
    {
        if (file_exists(LOGHOG_BASE_DIR . 'local/layout.php') && is_readable(LOGHOG_BASE_DIR . 'local/layout.php')) {
            $layoutConfig = json_decode(file_get_contents(LOGHOG_LOCAL_DIR . 'layout.json'));
            if (isset($layoutConfig->currentSelectedTheme)) {
                return $layoutConfig->currentSelectedTheme;
            }
            echoErrorJavaScript('', 'Error when getting current selected theme.', 9);
        } else {
            echoErrorJavaScript('',
                'Could not find local layout file. Please make sure that local/layout.php is setup correctly.', 7);
        }
    }
    ...
    function checkForStatusInstall($manualReturn, $relBase)
    {
        ...
        } elseif (is_dir(LOGHOG_BASE_DIR . '../status')) {
            return array(
                'local' => false,
                'loc' => $relBase . '../status/'
            );
    ...
    function checkForMonitorInstall($manualReturn, $relBase)
    {
        ...
        } elseif (is_file(LOGHOG_BASE_DIR . 'monitor/index.php')) {
            return array(
                'local' => true,
                'loc' => $relBase . 'monitor'
            );
        } elseif (is_dir(LOGHOG_BASE_DIR . '../monitor')) {
            return array(
                'local' => false,
                'loc' => $relBase . '../monitor/'
            );
    ...
    function checkForSearchInstall($manualReturn, $relBase)
    {
        ...
        } elseif (is_file(LOGHOG_BASE_DIR . 'search/index.php')) {
            return array(
                'local' => true,
                'loc' => $relBase . 'search/'
            );
        } elseif (is_dir(LOGHOG_BASE_DIR . '../search')) {
            return array(
                'local' => false,
                'loc' => $relBase . '../search/'
            );
    ...
    function checkForSeleniumMonitorInstall($manualReturn, $relBase)
    {
        ...
        } elseif (is_file(LOGHOG_BASE_DIR . 'seleniumMonitor/index.php')) {
            return array(
                'local' => true,
                'loc' => $relBase . 'seleniumMonitor/'
            );
        } elseif (is_dir(LOGHOG_BASE_DIR . '../seleniumMonitor')) {
            return array(
                'local' => false,
                'loc' => $relBase . '../seleniumMonitor/'
            );
